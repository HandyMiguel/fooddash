...
import { MessageCircle, Send, X, Search, CheckCheck, Clock, Users, Wifi, WifiOff, ArrowLeft } from 'lucide-react';
...
export default function AdminChat() {
  ...
  return (
    <div className="flex h-screen bg-black overflow-hidden">
      ...
      } md:translate-x-0 fixed md:relative z-40 w-full md:w-80 flex-shrink-0 bg-black border-r border-gray-800 flex flex-col transition-transform duration-300 h-full`}>
        <div className="px-4 py-4 border-b border-gray-800">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <MessageCircle className="w-5 h-5 text-orange-500" />
              <h1 className="font-semibold text-white">Live Chat</h1>
              {totalUnread > 0 && (
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1 text-xs">
                {connected
                  ? <><Wifi className="w-3.5 h-3.5 text-green-500" /><span className="text-green-600 hidden sm:inline">En ligne</span></>
                  : <><WifiOff className="w-3.5 h-3.5 text-gray-400" /><span className="text-gray-400 hidden sm:inline">Déco.</span></>
                }
              </div>
              <button
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-gray-900 border border-gray-700 rounded-xl pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 text-white placeholder-gray-500"
            />
          </div>
        </div>

        <div className="px-4 py-2 bg-orange-500/10 border-b border-orange-500/20 flex gap-4 text-xs text-orange-400">
          ...
          {filteredRooms.length === 0 ? (
            ...
            filteredRooms.map(room => (
              <div
                ...
                className={`flex items-start gap-3 px-4 py-3.5 cursor-pointer border-b border-gray-800 transition-all group hover:bg-orange-500/10 ${
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={`font-medium text-sm truncate ${room.unreadAdmin > 0 ? 'text-white' : 'text-gray-300'}`}>
                      {room.clientName}
                    </span>
                  ...
                  <p className={`text-xs truncate ${room.unreadAdmin > 0 ? 'text-gray-200 font-medium' : 'text-gray-500'}`}>
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-red-500/10 rounded text-red-400 transition"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
        ...
        {!activeRoom ? (
          <div className="flex-1 flex flex-col items-center justify-center text-gray-500 gap-4">
            ...
            <div className="text-center">
              <p className="font-medium text-gray-300">Sélectionnez une conversation</p>
              <p className="text-sm mt-1 text-gray-500">Choisissez un client dans la liste</p>
            </div>
          </div>
        ) : (
          <>
            {/* Header mobile avec retour */}
            <div className="bg-black border-b border-gray-800 px-4 sm:px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-white">{activeRoom.clientName}</p>
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-red-400 border border-red-500/20 px-3 py-1.5 rounded-lg hover:bg-red-500/10 transition"
              >
                <X className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Clôturer</span>
              </button>
            </div>
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id ?? i} className={`flex gap-3 ${isAdmin ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={`max-w-[75%] sm:max-w-[65%] flex flex-col gap-1 ${isAdmin ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                        isAdmin
                          ...
                          : 'bg-gray-900 text-gray-100 rounded-bl-sm border border-gray-700'
                      }`}>
                        <p className="whitespace-pre-wrap break-words">{msg.message}</p>
                      </div>
                      <span className={`text-[10px] text-gray-500 flex items-center gap-1 px-1 ${isAdmin ? 'flex-row-reverse' : ''}`}>
            ...
            <div className="px-4 sm:px-6 py-2 bg-black border-t border-gray-800">
              <div className="flex flex-wrap gap-2">
                {QUICK_ADMIN.map((r, i) => (
                  <button
                    key={i}
                    onClick={() => setInput(r)}
                    className="text-xs bg-gray-900 text-gray-400 px-3 py-1 rounded-full hover:bg-orange-500/10 hover:text-orange-400 transition border border-gray-700"
                  >
                    {r}
                  </button>
                ))}
            ...
            <div className="px-4 sm:px-6 py-4 bg-black border-t border-gray-800">
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  rows={2}
                  className="flex-1 bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-orange-400 text-white placeholder-gray-500"
                />
                <button
                  onClick={handleSend}
                  disabled={!input.trim() || !connected}
                  className="bg-gradient-to-br from-orange-500 to-red-500 disabled:from-gray-700 disabled:to-gray-700 text-white p-3 rounded-xl transition-all hover:scale-105 active:scale-95 flex-shrink-0 shadow-md shadow-orange-500/20"
                >
